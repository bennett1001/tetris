...
def convert_shape_format(shape):
    positions = []
    format = shape.shape[shape.rotation % len(shape.shape)] # rotate modulus
...
def draw_text_middle(text, size, color, surface):
    font = pygame.font.SysFont("Century", size, bold=True)
...
def main_menu(win):
    ...
    while run:
        win.fill((0,0,0))
        draw_text_middle("PRESS ANY KEY TO PLAY", 48, (255,255,255), win)
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
